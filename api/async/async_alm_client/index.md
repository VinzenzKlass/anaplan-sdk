...
This Class is not meant to be instantiated directly, but rather accessed through the `alm` Property on an instance of [AsyncClient](https://vinzenzklass.github.io/anaplan-sdk/api/async/async_client/index.md). For more details, see the [Guide](https://vinzenzklass.github.io/anaplan-sdk/guides/alm/index.md).
...
```
get_comparison_report(task: CompletedReportTask) -> bytes
```
...
| Name   | Type                  | Description                                    | Default    |
| ------ | --------------------- | ---------------------------------------------- | ---------- |
| `task` | `CompletedReportTask` | The report task object containing the task ID. | *required* |
