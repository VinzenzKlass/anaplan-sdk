...
def client(config: PyVersionConfig) -> Client:
    return Client(
...
@pytest.fixture(scope="session")
def client_small_pages(config: PyVersionConfig) -> Client:
    return Client(
        workspace_id=config.workspace_id,
        model_id=config.model_id,
        certificate=os.environ["ANAPLAN_SDK_TEST_CERT"],
...
@pytest.fixture(scope="session")
def alm_client(client: Client, config: PyVersionConfig) -> Client:
    return client.with_model(config.alm_model_id_sync)


@pytest.fixture(scope="session")
def alm_src_client(client: Client, config: PyVersionConfig) -> Client:
    return client.with_model(config.alm_src_model_id_sync)


@pytest.fixture(scope="session")
def test_integration_ids(config: PyVersionConfig) -> list[str]:
    return [
        config.test_integration_sync,
