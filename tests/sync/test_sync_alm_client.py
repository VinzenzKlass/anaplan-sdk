...
def test_change_model_status(alm_client: Client) -> None:
    alm_client.alm.change_model_status("offline")


def test_list_revisions(alm_client: Client) -> None:
    revisions = alm_client.alm.get_revisions()
    assert isinstance(revisions, list)
    assert all(isinstance(rev, Revision) for rev in revisions)


def test_get_latest_revision(alm_client: Client) -> None:
    rev = alm_client.alm.get_latest_revision()
    assert isinstance(rev, Revision)


def test_list_syncable_revisions(alm_client: Client, config: PyVersionConfig) -> None:
    revisions = alm_client.alm.get_syncable_revisions(config.alm_src_model_id_sync)
    assert isinstance(revisions, list)
    assert all(isinstance(rev, Revision) for rev in revisions)


def test_create_revision(alm_src_client: Client) -> None:
...
def test_create_comparison_summary_task(
    alm_client: Client, alm_src_client: Client, config: PyVersionConfig
) -> None:
    src_rev, latest_rev = (
    ...
    report_task = alm_client.alm.create_comparison_summary(
        src_rev.id, config.alm_src_model_id_sync, latest_rev.id, False
    )
...
def test_create_comparison_summary(
    ...
    assert isinstance(src_rev, Revision)
    assert isinstance(latest_rev, Revision)
    report = alm_client.alm.create_comparison_summary(
        src_rev.id, config.alm_src_model_id_sync, latest_rev.id
    )
...
def test_sync_models(alm_client: Client, alm_src_client: Client, config: PyVersionConfig) -> None:
    src_rev, latest_rev = (
        alm_src_client.alm.get_latest_revision(),
        alm_client.alm.get_latest_revision(),
    )
    assert isinstance(src_rev, Revision)
    assert isinstance(latest_rev, Revision)
    sync_task = alm_client.alm.sync_models(
        src_rev.id, config.alm_src_model_id_sync, latest_rev.id, True
    )
...
def test_list_models_for_revision(alm_client: Client) -> None:
    rev = alm_client.alm.get_latest_revision()
    assert isinstance(rev, Revision)
...
def test_list_sync_tasks(alm_client: Client) -> None:
