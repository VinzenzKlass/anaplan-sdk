...
async def test_create_comparison_summary_task(
    ...
    report_task = await alm_client.alm.create_comparison_summary(
        src_rev.id, config.alm_src_model_id_async, latest_rev.id, False
    )
...
async def test_create_comparison_summary(
    ...
    report = await alm_client.alm.create_comparison_summary(
        src_rev.id, config.alm_src_model_id_async, latest_rev.id
    )
...
async def test_sync_models(
    ...
    assert isinstance(src_rev, Revision)
    assert isinstance(latest_rev, Revision)
    sync_task = await alm_client.alm.sync_models(
        src_rev.id, config.alm_src_model_id_async, latest_rev.id, True
    )
